...
module module_MEDIATOR_methods
  ...
  public shr_flux_atmOcn, shr_flux_atmOcn_bf
...
! !BOP =========================================================================
!
! !IROUTINE: shr_flux_atmOcn_bf -- internal atm/ocn flux calculation
!                                  with inputs of u10m, v10m, t2m, q2m, taux, taut
!
! !DESCRIPTION:
!
!     Internal atm/ocn flux calculation
!     Provided by CESM September 2015
!     Modified by EMC/NCEP JULY 2020 
!     
! !REVISION HISTORY:
!
! !INTERFACE: ------------------------------------------------------------------

!SUBROUTINE shr_flux_atmOcn_bf(nMax  ,zbot  ,ubot  ,vbot  ,thbot ,   & 
!           &               qbot  ,rbot  ,tbot  ,us    ,vs    ,   &
!           &               ts    ,mask  ,sen   ,lat   ,lwup  ,   &
!           &               evap  ,taux  ,tauy  ,tref  ,qref  ,   &
!           &               duu10n,  ustar_sv   ,re_sv ,ssq_sv,   &
!           &               missval    )
SUBROUTINE shr_flux_atmOcn_bf(nMax  ,u10m  ,v10m  ,t2m  ,th2m ,q2m ,   & 
           &               zbot,  rbot  ,thbot  ,tbot  ,qbot  ,  &
           &               us    ,vs    ,ts   ,mask  ,   &
           &               sen   ,lat   ,lwup  ,                 &
           &               evap  ,taux  ,tauy  ,tref  ,qref  ,   &
           &               duu10n,  ustar_sv   ,re_sv ,ssq_sv,   &
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   integer(IN),intent(in) ::       nMax  ! data vector length
   integer(IN),intent(in) :: mask (nMax) ! ocn domain mask       0 <=> out of domain
   real(R8)   ,intent(in) :: u10m (nMax) ! atm 10m u wind        (m/s)
   real(R8)   ,intent(in) :: v10m (nMax) ! atm 10m v wind        (m/s)
   real(R8)   ,intent(in) :: t2m (nMax)  ! atm 2m T              (K)
   real(R8)   ,intent(in) :: th2m (nMax) ! atm 2m potential      (K)
   real(R8)   ,intent(in) :: q2m (nMax)  ! atm 2m specific humidity (kg/kg)
   real(R8)   ,intent(in) :: zbot (nMax) ! atm level height      (m)
   real(R8)   ,intent(in) :: rbot (nMax) ! atm air density       (kg/m^3)
   real(R8)   ,intent(in) :: thbot (nMax) ! atm potential T      (K) 
   real(R8)   ,intent(in) :: tbot (nMax) ! atm T                 (K) 
...
! !EOP
   ...
   !--- local variables --------------------------------
   integer(IN) :: n      ! vector loop index
   real(R8)    :: vmag   ! 10m wind magnitude   (m/s)
   real(R8)    :: thvbot ! virtual temperature      (K)
   real(R8)    :: ssq    ! sea surface humidity     (kg/kg)
   real(R8)    :: delt   ! potential T difference   (K)
   real(R8)    :: deltst ! 2m T difference between air and SST  (K)
   real(R8)    :: delq   ! humidity difference      (kg/kg)
   real(R8)    :: stable ! stability factor
   real(R8)    :: rdn    ! sqrt of neutral exchange coeff (momentum) 
   real(R8)    :: rhn    ! sqrt of neutral exchange coeff (heat)     
...
!-------------------------------------------------------------------------------
   ...
   DO n=1,nMax
     if (mask(n) /= 0) then
        ...
        !--- compute some needed quantities ---
        vmag   = max(umin, sqrt( (u10m(n)-us(n))**2 + (v10m(n)-vs(n))**2) )
        thvbot = th2m(n) * (1.0_R8 + shr_const_zvir * q2m(n)) ! virtual temp (K)
        ssq    = 0.98_R8 * qsat(ts(n)) / rbot(n)   ! sea surf hum (kg/kg)
        delt   = th2m(n) - ts(n)                  ! pot temp diff (K)
   !    deltst = t2m(n) - (ts(n) - 0.012)         ! at 2m t2m:env temp) te(n) 0.012:2m up 
        ...
        !---
        stable = 0.5_R8 + sign(0.5_R8 , deltst)
        rdn    = sqrt(cdn(vmag))
        rhn    = (1.0_R8-stable) * 0.0327_R8 + stable * 0.018_R8 
        ren    = 0.0346_R8 
        ...
        !--- momentum flux ---
        taux(n) = tau * (u10m(n)-us(n)) / vmag 
        tauy(n) = tau * (v10m(n)-vs(n)) / vmag 
...
END subroutine shr_flux_atmOcn_bf
